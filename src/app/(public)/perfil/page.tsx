...
import { AtSign, ArrowRight, Clock } from "lucide-react"
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const name = profile?.name ?? "El artista"

  return {
    title: `${name} — Artista`,
    description: profile?.bio?.slice(0, 160) ?? "Tatuador profesional",
  }
}
...
export default async function PerfilPage() {
  ...
  const instagram = profile?.instagramHandle ?? null
      ...
      <section aria-labelledby="artist-name-heading" className="bg-background px-6 py-16 md:py-24">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-10 items-start">
          ...
          <div className="flex-1">
            <h1 id="artist-name-heading" className="text-h1 text-foreground mb-6">
              {name}
            </h1>
            ...
            <dl className="flex flex-col gap-2 mb-8">
              {yearsOfExperience !== null && (
                <div className="flex items-center gap-2 text-sm text-foreground-secondary">
                  ...
                  <dd>{yearsOfExperience} años de experiencia</dd>
                </div>
              )}
              {instagram && (
                <div className="flex items-center gap-2 text-sm text-foreground-secondary">
